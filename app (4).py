...
# ── Session state portfolios — editable in app ────────────────────────────────
if "my_portfolio" not in st.session_state:
    st.session_state["my_portfolio"] = dict(PORTFOLIO)
if "dad_portfolio" not in st.session_state:
    st.session_state["dad_portfolio"] = dict(DAD_PORTFOLIO)

# Always use session state versions so edits propagate everywhere
PORTFOLIO     = st.session_state["my_portfolio"]
DAD_PORTFOLIO = st.session_state["dad_portfolio"]
...
with st.sidebar:
    ...
    page = st.radio("", ["Chart", "My Portfolio", "Dad's Portfolio",
                         ...
                         "Options Flow", "Portfolio Manager"],
...
# ═══════════════════════════════════════════════════════════════════════════════
#  PAGE: OPTIONS FLOW SCANNER
# ═══════════════════════════════════════════════════════════════════════════════

elif page == "Options Flow":
    from data.options_fetcher import (get_options_expirations, get_options_chain,
                                      get_unusual_flow, get_whale_prints,
                                      get_putcall_ratio, fmt_premium)
    from charts.options_plots import (volume_oi_chart, premium_bar,
                                      iv_smile, putcall_bar)

    st.markdown("# Options Flow Scanner")
    st.caption("Unusual activity, whale prints, and put/call sentiment — powered by Yahoo Finance options data.")

    # Controls
    flow_tickers = [t for t in list(PORTFOLIO.keys()) + ["SPY","QQQ","NVDA","TSLA","AAPL","MSTR","AMD","META"]
                    if t not in ["STRK"]]
    flow_tickers = list(dict.fromkeys(flow_tickers))  # dedupe

    fc1, fc2, fc3, fc4 = st.columns(4)
    with fc1:
        flow_ticker = st.selectbox("Ticker", flow_tickers, key="flow_tick")
    with fc2:
        min_premium_k = st.number_input("Min Premium ($K)", value=100, step=50, key="flow_prem")
        min_premium   = min_premium_k * 1000
    with fc3:
        min_vol_oi = st.slider("Min Vol/OI Ratio", 1.0, 10.0, 2.0, 0.5, key="flow_voi")
    with fc4:
        max_exp = st.slider("Expirations to scan", 2, 12, 6, key="flow_exp")

    scan_btn = st.button("▶ Scan Flow")

    if scan_btn or True:
        with st.spinner(f"Scanning {flow_ticker} options flow..."):
            expirations = get_options_expirations(flow_ticker)
            pc_ratio    = get_putcall_ratio(flow_ticker)
            unusual     = get_unusual_flow(flow_ticker, min_vol_oi, min_premium, max_exp)
            whales      = get_whale_prints(flow_ticker, min_premium, max_exp)
            chain_near  = get_options_chain(flow_ticker, expirations[0]) if expirations else {}

        # ── Sentiment strip ───────────────────────────────────────────────────
        if pc_ratio:
            sentiment_color = T["green"] if pc_ratio["sentiment"] == "Bullish" else \
                              T["red"]   if pc_ratio["sentiment"] == "Bearish" else T["ink_mid"]
            s1,s2,s3,s4,s5,s6 = st.columns(6)
            s1.metric("Sentiment",     pc_ratio["sentiment"])
            s2.metric("P/C Vol",       f"{pc_ratio['pc_ratio_vol']:.2f}")
            s3.metric("P/C OI",        f"{pc_ratio['pc_ratio_oi']:.2f}")
            s4.metric("Call Volume",   f"{pc_ratio['call_volume']:,}")
            s5.metric("Put Volume",    f"{pc_ratio['put_volume']:,}")
            s6.metric("Call Premium",  fmt_premium(pc_ratio["call_premium"]))

        st.markdown("---")

        # ── Whale prints table ────────────────────────────────────────────────
        st.markdown("### 🐋 Whale Prints")
        st.caption(f"Single strikes with >{fmt_premium(min_premium)} premium — sorted by size")

        if not whales.empty:
            display_cols = ["side","expiration","strike","lastPrice","volume",
                           "openInterest","vol_oi_ratio","premium_spent","impliedVolatility"]
            display_cols = [c for c in display_cols if c in whales.columns]
            whale_disp = whales[display_cols].head(20).copy()
            whale_disp["premium_spent"]     = whale_disp["premium_spent"].apply(fmt_premium)
            whale_disp["impliedVolatility"] = whale_disp["impliedVolatility"].apply(lambda x: f"{x:.1f}%")
            whale_disp["side"]              = whale_disp["side"].str.upper()
            whale_disp.columns = [c.replace("_"," ").title() for c in whale_disp.columns]
            st.dataframe(whale_disp, use_container_width=True, hide_index=True)
        else:
            st.info(f"No whale prints found above {fmt_premium(min_premium)}. Lower the minimum premium.")

        st.markdown("---")

        # ── Unusual flow table ────────────────────────────────────────────────
        st.markdown("### ⚡ Unusual Flow")
        st.caption(f"Vol/OI ratio ≥ {min_vol_oi}x — new money entering, not existing positions trading")

        if not unusual.empty:
            u_cols = ["side","expiration","strike","lastPrice","volume",
                      "openInterest","vol_oi_ratio","premium_spent","impliedVolatility"]
            u_cols = [c for c in u_cols if c in unusual.columns]
            u_disp = unusual[u_cols].head(20).copy()
            u_disp["premium_spent"]     = u_disp["premium_spent"].apply(fmt_premium)
            u_disp["impliedVolatility"] = u_disp["impliedVolatility"].apply(lambda x: f"{x:.1f}%")
            u_disp["side"]              = u_disp["side"].str.upper()
            u_disp.columns = [c.replace("_"," ").title() for c in u_disp.columns]
            st.dataframe(u_disp, use_container_width=True, hide_index=True)
        else:
            st.info(f"No unusual flow found. Try lowering Vol/OI ratio or minimum premium.")
        ...
        with ch1:
            if not whales.empty:
                st.plotly_chart(premium_bar(whales, flow_ticker),
                                use_container_width=True, key="opt_prem")
        with ch2:
            if pc_ratio:
                st.plotly_chart(putcall_bar(pc_ratio, flow_ticker),
                                use_container_width=True, key="opt_pc")

        ch3, ch4 = st.columns(2)
        with ch3:
            if not whales.empty:
                st.plotly_chart(volume_oi_chart(whales, flow_ticker, "call"),
                                use_container_width=True, key="opt_vol_call")
        with ch4:
            if not whales.empty:
                st.plotly_chart(volume_oi_chart(whales, flow_ticker, "put"),
                                use_container_width=True, key="opt_vol_put")

        # IV Smile
        if chain_near and not chain_near["calls"].empty:
            st.plotly_chart(iv_smile(chain_near, flow_ticker, expirations[0]),
                            use_container_width=True, key="opt_iv")

        st.markdown("---")
        st.caption("Data: Yahoo Finance · 15min delayed · Vol/OI > 2x = unusual · Not financial advice")
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    st.caption("Add, edit or remove positions. Changes apply immediately across all pages for this session.")

    def render_editor(port_key: str, label: str, exclude_key: str):
        st.markdown(f"### {label}")
        port = st.session_state[port_key]

        # ── Existing positions ────────────────────────────────────────────────
        st.markdown("**Current Positions**")
        to_delete = []
        for ticker, pos in list(port.items()):
            col1, col2, col3, col4 = st.columns([1, 1.5, 1.5, 0.8])
            col1.markdown(f"**{ticker}**")
            new_qty  = col2.number_input("Qty",      value=float(pos["qty"]),
                                          min_value=0.0, step=1.0,
                                          key=f"{port_key}_{ticker}_qty",
                                          label_visibility="collapsed")
            new_cost = col3.number_input("Avg Cost", value=float(pos["avg_cost"]),
                                          min_value=0.0, step=0.01,
                                          key=f"{port_key}_{ticker}_cost",
                                          label_visibility="collapsed")
            if col4.button("✕ Remove", key=f"{port_key}_{ticker}_del"):
                to_delete.append(ticker)
            else:
                port[ticker] = {"qty": new_qty, "avg_cost": new_cost}
        ...
        a1, a2, a3, a4 = st.columns([1, 1.5, 1.5, 1])
        new_ticker = a1.text_input("Ticker", placeholder="e.g. COIN",
                                    key=f"{port_key}_new_tick").upper().strip()
        new_qty    = a2.number_input("Shares", value=10.0, min_value=0.01,
                                      step=1.0, key=f"{port_key}_new_qty",
                                      label_visibility="collapsed")
        new_cost   = a3.number_input("Avg Cost ($)", value=100.0, min_value=0.01,
                                      step=0.01, key=f"{port_key}_new_cost",
                                      label_visibility="collapsed")
        if a4.button("＋ Add", key=f"{port_key}_add_btn"):
            if new_ticker:
                port[new_ticker] = {"qty": new_qty, "avg_cost": new_cost}
                st.success(f"Added {new_ticker} — {new_qty} shares @ ${new_cost:.2f}")
                st.rerun()

        # ── Exclude from returns ──────────────────────────────────────────────
        st.markdown("**Exclude from Returns Chart**")
        st.caption("Tickers excluded from the cumulative return vs SPY chart (too new or illiquid)")
        current_excl = list(st.session_state.get(exclude_key, RETURNS_EXCLUDE if port_key == "my_portfolio" else DAD_RETURNS_EXCLUDE))
        excl_input = st.text_input("Excluded tickers (comma separated)",
                                    value=", ".join(current_excl),
                                    key=f"{port_key}_excl")
        st.session_state[exclude_key] = [t.strip().upper() for t in excl_input.split(",") if t.strip()]

        st.markdown("---")

        # ── Summary ───────────────────────────────────────────────────────────
        st.markdown(f"**{label} Summary** — {len(port)} positions")
        if port:
            sum_df = pd.DataFrame([
                {"Ticker": t, "Qty": p["qty"], "Avg Cost": f"${p['avg_cost']:.2f}",
                 "Cost Basis": f"${p['qty']*p['avg_cost']:,.0f}"}
                for t, p in port.items()
            ])
            st.dataframe(sum_df, use_container_width=True, hide_index=True)

    # Render both portfolio editors
    tab1, tab2 = st.tabs(["My Portfolio", "Dad's Portfolio"])

    with tab1:
        render_editor("my_portfolio", "My Portfolio", "my_excl")
        # Sync back to global
        PORTFOLIO = st.session_state["my_portfolio"]

    with tab2:
        render_editor("dad_portfolio", "Dad's Portfolio", "dad_excl")
        DAD_PORTFOLIO = st.session_state["dad_portfolio"]

    st.info("💡 Changes apply to this session only. To make permanent, update config.py with your new positions.")
